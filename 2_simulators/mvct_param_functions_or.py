...
def levy_prior(fix_alpha, batch_size):
    ...
    p_samples = np.c_[t0, st0, zr, v0, v1, a, alpha]
...
@jit(nopython=True)
def diffusion_trial(v, a, ndt, sndt, zr, alpha, deadline):
    n_steps = 0.
    dt = 0.001
    max_steps = 10000
...
def ddm_simulator(theta, n_obs, deadline):
    ...
    for n in range(n_obs):
        out[n] = diffusion_trial(v[design_matrix[n]], a=theta[5], ndt=theta[0],sndt=theta[1], zr=theta[2], alpha=theta[6], deadline=deadline)
...
def random_num_obs(min_obs=317, max_obs=352): 
