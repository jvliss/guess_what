...
async function experimentInit() {
  ...
  if (((expInfo["participant"] % 2) == 0)) {
      group = groups[1]; //guess
  } else {
      group = groups[0]; //control
  }
  console.log("group:", group)
...
function prepRoutineBegin(snapshot) {
  return async function () {
    ...
    if (block_i < 5){  
        ...
        "You will now be forwarded to a short questionnaire." +  "\n\n" +
        'For this, you will be redirected to a new page after pressing the spacebar. Please wait for the pop-up window that says, "Terminating the experiment. Please wait a few seconds," to disappear. Then, click "OK" on the subsequent pop-up window.' +
...
function rt_trialRoutineEnd(snapshot) {
  return async function () {
    ...
    if (key_resp.rt < 0.51 & trial_type != 'attention') { //Fast guess
        if (block_i == 0) {
            feedback_text += "\nToo fast!"
        } else {
