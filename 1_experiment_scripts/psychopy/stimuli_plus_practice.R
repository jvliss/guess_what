##this code creates 4 new practice trials and checks that it does not double with any trials from the stimulus pool previously used
##practice trials are the added to the stimuli pool
##n of practice trials was not increased in old script because missing seed would lead to completely different stimuli 
#install packages
library(tidyverse)
library(writexl)
library(jsonlite)
...
setwd("C:/Users/Katharina Mminele/iCloudDrive/Manuskript")
load("stimuli.rda")
stimuli <- df
...
while (dupls == 1){
  ...
  for (cond in conditions){
    ...
    if(cond == "practice"){
      ...
      order <- NA
      for(i in 1:(nrow(df)/2)){#creates order of alternating 450 & 550; impotant for split and spacing of attention checks -> lines 214ff
...
for(i in 1:nrow(stimuli)){
  ...
  for(k in 1:nrow(df)){
    b <- df[k,]
    duplicates <- c(duplicates, setequal(a,b))
  }
}

stimuli_new <- rbind(df, stimuli)
stimuli_new <- rbind(
  stimuli_new[sample(1:8), ],  # new random order for 1-8
  stimuli_new[9:nrow(stimuli_new), ]    # order of other trials remains the same as in prvious study
)

#clean up
rm(list = ls()[!ls() %in% c("df", "stimuli", "stimuli_new", "time.taken")])
...
table(stimuli_new$block)
view(stimuli_new)

# save stimuli as rda
save(df, file = "stimuli_new.rda")

#write json

out = c(list(practice = as.list(stimuli_new[stimuli_new[,"block"]=="0",]), 
             burn_in = as.list(stimuli_new[stimuli_new[,"block"]=="1",]), 
             test_1 = as.list(stimuli_new[stimuli_new[,"block"]=="2",]), 
             test_2 = as.list(stimuli_new[stimuli_new[,"block"]=="3",]), 
             test_3 = as.list(stimuli_new[stimuli_new[,"block"]=="4",]), 
             test_4 = as.list(stimuli_new[stimuli_new[,"block"]=="5",])))

names(out) <- NULL
write_json(out, 'stimuli_new.json', pretty=T)
